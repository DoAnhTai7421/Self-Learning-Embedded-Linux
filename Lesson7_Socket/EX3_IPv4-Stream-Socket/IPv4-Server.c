...
{       
    ...
    while (1) {        
        ...
        if(numb_read == -1)
            handle_error("read()");
        if (strncmp("exit", recvbuff, 4) == 0) {                //Nếu client gửi "exit" thì server sẽ đóng kết nối
            system("clear");
            break;
        }
...
int main(int argc, char *argv[])
{
    ...
    serv_addr.sin_addr.s_addr =  INADDR_ANY; //Server sẽ lắng nghe trên tất cả các địa chỉ IP của máy //INADDR_ANY
